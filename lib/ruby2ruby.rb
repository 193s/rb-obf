...
class Ruby2Ruby < SexpProcessor
  ...
  def process_block(exp) # :nodoc:
    ...
    result += ";" unless result.start_with? "("
  ...
  def process_if(exp) # :nodoc:
    ...
    if t then
      ...
      #r = "if #{c} then\n#{indent(t)}\n"
      if c[0] == '(' && c[-1] == ')'
        r = "if#{c}then;#{indent(t)};"
      else
        r = "if #{c} then;#{indent(t)};"
      end
      #r << "else\n#{indent(f)}\n" if f
      r << "else;#{indent(f)};" if f
      r << "end"
  ...
  def process_str(exp) # :nodoc:
    ...
    if val == ''
      return '%()'
    elsif val.length == 1 && /[0-9a-zA-Z!"#$%&'()-=^~|?]/.match(val)
      return ('?'+val)
    else
      return val.dump
    end
